...
# Tout le réseau : VPC, subnets, IGW, NAT, route tables, SG bastion.
# = main.tf du TP02 renommé en vpc.tf pour séparer réseau et EC2.
...
# 1 subnet public par AZ, avec IP publique automatique
...
# 1 subnet privé par AZ, pas d'IP publique
...
# NAT en eu-west-3b (azs[1]) car eu-west-3a saturée en TP02
# Permet aux EC2 privées de sortir sur Internet sans être joignables
...
# RT privée : trafic sortant -> NAT (Internet via NAT, pas direct)
...
# Associations RT <-> subnets (sans ça, subnet utilise la RT par défaut = pas d'Internet)
